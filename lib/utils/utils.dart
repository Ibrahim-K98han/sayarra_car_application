...
class Utils {
  ...
  static Uri tokenWithCodeSearch(
    ...
    if (carType.isNotEmpty) {
      for (int i = 0; i < carType.length; i++) {
        queryParams["car_types[$i]"] = carType[i];
      }
    }

    if (bodyType.isNotEmpty) {
      for (int i = 0; i < bodyType.length; i++) {
        queryParams["body_type_id[$i]"] = bodyType[i];
      }
    }

    if (bodyType.isNotEmpty) {
      for (int i = 0; i < bodyType.length; i++) {
        queryParams["body_types[$i]"] = bodyType[i];
      }
    }

    if (fuelType.isNotEmpty) {
      for (int i = 0; i < fuelType.length; i++) {
        queryParams["fuel_types[$i]"] = fuelType[i];
      }
    }

    if (transmission.isNotEmpty) {
      for (int i = 0; i < transmission.length; i++) {
        queryParams["transmissions[$i]"] = transmission[i];
      }
    }

    if (interiorColor.isNotEmpty) {
      for (int i = 0; i < interiorColor.length; i++) {
        queryParams["interior_colors[$i]"] = interiorColor[i];
      }
    }

    if (exteriorColor.isNotEmpty) {
      for (int i = 0; i < exteriorColor.length; i++) {
        queryParams["exterior_colors[$i]"] = exteriorColor[i];
      }
    }

    if (mileage.isNotEmpty) {
      for (int i = 0; i < mileage.length; i++) {
        queryParams["mileages[$i]"] = mileage[i];
      }
    }

    if (numberOfSeat.isNotEmpty) {
      for (int i = 0; i < numberOfSeat.length; i++) {
        queryParams["number_of_seats[$i]"] = numberOfSeat[i];
      }
    }
  ...
  static String wordLength(String text, int maxLength) {
    text = text.trim();
    if (text.length <= maxLength) return text;

    int lastSpace = text.substring(0, maxLength).lastIndexOf(' ');

    // If the space is too early (e.g. "1901 "), ignore it
    if (lastSpace < maxLength / 2) {
      return text.substring(0, maxLength).trimRight();
    }

    return text.substring(0, lastSpace).trimRight();
  }
  ...
  static void closeUpdateDialog(BuildContext context) {
  ...
  static String formatYear(String timestamp) {
    // Parse the timestamp into a DateTime object
    DateTime dateTime = DateTime.parse(timestamp);

    // Format the date and time
    // String formattedDate = DateFormat('yyyy-MM-dd').format(dateTime); // Example: 2024-11-20
    String formattedYear = DateFormat('yyyy').format(dateTime); // Example: Mar 03 2024
